...
export function sanitizeSportsRound(raw, { expectedMatches } = {}) {
  ...
  const round = {
    ...
    matches,
    prizeCategories: Array.isArray(raw?.prizeCategories)
      ? raw.prizeCategories.map(category => ({
          category: cleanText(category?.category || category?.label || category?.name, 160),
          winners: Number.isInteger(Number(category?.winners)) && Number(category.winners) >= 0 ? Number(category.winners) : null,
          prize: Number.isFinite(Number(category?.prize ?? category?.amount)) ? Number(category?.prize ?? category?.amount) : null,
        })).filter(category => category.category)
      : Array.isArray(raw?.metadata?.prizeCategories)
        ? raw.metadata.prizeCategories.map(category => ({
            category: cleanText(category?.category || category?.label || category?.name, 160),
            winners: Number.isInteger(Number(category?.winners)) && Number(category.winners) >= 0 ? Number(category.winners) : null,
            prize: Number.isFinite(Number(category?.prize ?? category?.amount)) ? Number(category?.prize ?? category?.amount) : null,
          })).filter(category => category.category)
