...
function euroDreams(ticket, results) {
  ...
  if(m===6&&dream) return result('1ª categoria (6 + Sueño)',m,null,'20.000 € al mese per 30 anni','deferred',{extraMatch:true});
  ...
  if(map[m]) { const amount=findOfficialPrize(results,map[m][1]); return result(map[m][0],m,amount,null,amount==null?'variable':'cash'); }
  return result(null,m,0,'Nessun premio','cash');
}
function primitiva(ticket, results) {
  ...
  if(m===3) return result('5ª categoria (3 numeri)',m,findOfficialPrize(results,['5']),'Premio variabile','variable');
  if(reintegro) return result('Reintegro',m,findOfficialPrize(results,['reintegro']) ?? 1,'1,00 €','cash',{extraMatch:true});
  return result(null,m,0,'Nessun premio','cash');
}
export function calculatePayout(ticket, results){ return ticket.gameId==='primitiva'?primitiva(ticket,results):euroDreams(ticket,results); }
