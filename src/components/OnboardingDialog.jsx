...
const STEPS = [
  { icon: PlusIcon, mascot: 'helper', title: 'Tu próxima jugada empieza aquí', text: 'Elige el juego y el número de columnas. Primy Core prepara una selección válida y ordenada en pocos instantes.' },
  { icon: TicketIcon, mascot: 'thinking', title: 'Guárdala a tu manera', text: 'Conserva una jugada como borrador o márcala como boleto comprado para encontrarla después sin esfuerzo.' },
  { icon: DatabaseIcon, mascot: 'celebration', title: 'Vuelve cuando llegue el sorteo', text: 'Primy mantiene tus jugadas organizadas y las compara con los resultados disponibles cuando llega el momento.' },
  { icon: ShieldIcon, mascot: 'responsible', title: 'Privado, claro y responsable', text: 'Tus jugadas pertenecen a tu cuenta. Primy no vende boletos, no predice sorteos y está pensado para mayores de edad.' },
];
  ...
  return (
    <AccessibleDialog open={open} onClose={finish} labelledBy="onboarding-title" className="sm:max-w-2xl" closeOnBackdrop={false}>
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-sm font-bold text-primy-700">Bienvenido a Primy</p><h2 id="onboarding-title" className="mt-1 text-2xl font-semibold text-primary">Tus jugadas, a tu manera</h2></div>
      ...
      <div className="mt-6 flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => step === STEPS.length - 1 ? finish() : setStep(current => current + 1)} className="min-h-11 rounded-xl bg-primy-700 px-5 text-sm font-semibold text-white hover:bg-primy-800">{step === STEPS.length - 1 ? 'Entrar en Primy' : 'Continuar'}</button>
        </div>
      </div>
    </AccessibleDialog>
  );
