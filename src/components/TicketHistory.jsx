...
export default function TicketHistory({ plays, onPurchase, onRemove, onSetPrize, onFavorite, onRepeat, onVariant }) {
  ...
  if (!filtered.length) return <section>{filters}<div className="mt-5 grid gap-5 rounded-[2rem] border border-dashed border-primy-200 bg-gradient-to-br from-white via-[#fcfefc] to-[#f4fbf7] p-6 md:grid-cols-[minmax(0,1fr)_300px] md:p-8"><div className="flex flex-col justify-center"><p className="text-sm font-bold text-primy-700">Tu espacio está listo</p><p className="mt-2 text-2xl font-semibold text-primary">Todavía no hay jugadas que mostrar</p><p className="mt-3 max-w-xl text-sm leading-7 text-secondary">Cambia los filtros o crea una jugada nueva. Primy te ayudará a organizar tus boletos y comprobarlos cuando llegue el sorteo.</p></div><PrimyMascotGraphic className="w-full max-w-[300px] justify-self-center" size="dashboard" caption="Cuando tengas jugadas, las guardaré aquí"/></div></section>;
