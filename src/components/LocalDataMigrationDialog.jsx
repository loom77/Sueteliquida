...
export default function LocalDataMigrationDialog({ count, onImport, onSkip, busy }) {
  return (
    <AccessibleDialog open={count > 0} onClose={() => {}} labelledBy="local-migration-title" closeOnBackdrop={false} className="sm:max-w-2xl">
      <div className="grid items-center gap-5 sm:grid-cols-[220px_minmax(0,1fr)]">
        <PrimyMascotGraphic className="mx-auto w-full max-w-[220px]" variant={busy ? "thinking" : "helper"} size="dashboard" compact showCaption={false}/>
        <div>
          <p className="text-sm font-semibold text-primy-700">Migración de datos</p>
