  ...
  return (
    <section className="rounded-2xl border border-default bg-surface p-4 md:p-5" aria-labelledby="distribution-title">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h3 id="distribution-title" className="font-semibold text-primary">Distribución de las columnas</h3>
        ...
        <p className="text-sm font-bold text-primy-700">{covered}/{game.numberPoolMax} números cubiertos</p>
      </div>
        ...
        {counts.slice(1).map((count, index) => {
          ...
          return (
            <div
              ...
              className="relative flex aspect-square items-center justify-center rounded-lg border border-default text-xs font-semibold text-primary"
              style={count ? { backgroundColor: `color-mix(in srgb, var(--primy-accent) ${Math.round(intensity * 100)}%, transparent)` } : undefined}
            >
              {number}
              {count > 1 && <span className="absolute right-0.5 top-0.5 rounded bg-primy-800 px-1 text-xs leading-4 text-white">{count}</span>}
            </div>
          );
        })}
      </div>
