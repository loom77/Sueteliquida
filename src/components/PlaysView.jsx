...
export default function PlaysView({ now, plays, dueByGame, verificationError, checkingGame, checkingPlayId, onCheck, onCheckPlay, onPurchase, onRemove, onSetPrize, onFavorite, onRepeat, onVariant, onAddExternal, onCreate }) {
  ...
  return (
    <div className="primy-page-enter mx-auto max-w-[1380px] space-y-5 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <TicketHistory now={now} plays={plays} checkingGame={checkingGame} checkingPlayId={checkingPlayId} onCheckPlay={onCheckPlay} onCreate={onCreate} onAddExternal={onAddExternal} onPurchase={onPurchase} onRemove={onRemove} onSetPrize={onSetPrize} onFavorite={onFavorite} onRepeat={onRepeat} onVariant={onVariant}/>
    </div>
  );
}
