...
export default function PlaysView({ plays, dueByGame, verificationError, checkingGame, onCheck, onPurchase, onRemove, onSetPrize, onFavorite, onRepeat, onVariant, onAddExternal, onCreate }) {
  ...
  return (
    <div className="primy-page-enter mx-auto max-w-[1380px] space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-primy-700">Archivo</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-primary">Tus jugadas, siempre organizadas</h1><p className="mt-2 text-sm leading-6 text-secondary">Consulta borradores, boletos registrados y resultados desde un único lugar.</p></div><button type="button" onClick={onAddExternal} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-primy-700 px-5 text-sm font-semibold text-white hover:bg-primy-800"><EditIcon width="18" height="18"/>Añadir boleto</button></div>
