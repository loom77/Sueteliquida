...
export default function GeneratorPanel({ game, activeGame, onGameChange, columnCount, setColumnCount, onGenerate, busy, progress = 0, generationError, monthlySpent = 0, monthlyLimit = null, variantLabel = '', onClearVariant }) {
  ...
  return (
    <section className="primy-panel p-5 md:p-7">
      <div className="flex items-start gap-4">
        <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${isPrimitiva ? 'bg-primy-100 text-primy-800' : 'bg-violet-100 text-violet-800'}`}><SparklesIcon width="22" height="22"/></span>
        <div><p className={`text-sm font-semibold ${isPrimitiva ? 'text-primy-700' : 'text-violet-700'}`}>Paso 2 de 2</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-primary">Elige cuánto quieres jugar</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-secondary">Una sola decisión: cuántas columnas quieres generar.</p></div>
      </div>

      {variantLabel && <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-violet-200 bg-violet-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-violet-950">Variante de una jugada existente</p><p className="mt-1 text-sm leading-6 text-violet-800">Primy buscará columnas distintas de {variantLabel}.</p></div><button type="button" onClick={onClearVariant} className="min-h-11 rounded-xl px-4 text-sm font-semibold text-violet-800 hover:bg-violet-100">Quitar referencia</button></div>}

      <div className="mt-7"><GameSwitch active={activeGame} onChange={onGameChange} label="Juego elegido"/></div>

      <div className={`mt-6 overflow-hidden rounded-3xl border p-5 sm:p-6 ${isPrimitiva ? 'border-primy-200 bg-gradient-to-br from-primy-50 to-cream dark:from-primy-950 dark:to-surface' : 'border-violet-200 bg-gradient-to-br from-violet-50 to-cream dark:from-violet-950 dark:to-surface'}`}>
        ...
        <div className="mt-6 flex items-center justify-between gap-4 rounded-2xl border border-white/70 bg-surface p-3 shadow-sm sm:p-4">
      ...
      <button type="button" onClick={onGenerate} disabled={busy} className={`mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl px-6 text-base font-semibold text-white shadow-soft disabled:cursor-not-allowed disabled:opacity-60 ${isPrimitiva ? 'bg-primy-700 hover:bg-primy-800' : 'bg-eurodreams hover:bg-violet-800'}`}>
        <SparklesIcon width="20" height="20"/>{busy ? `Preparando · ${Math.round(progress * 100)}%` : `Generar ${columnCount === 1 ? 'la jugada' : `${columnCount} columnas`} · ${euro.format(totalCost)}`}
      </button>
      <p className="mt-3 text-center text-xs leading-5 text-secondary">La generación no guarda ni compra nada.</p>

      {busy && <div className="mt-4" role="progressbar" aria-label="Progreso de generación" aria-valuemin="0" aria-valuemax="100" aria-valuenow={Math.round(progress * 100)}><div className="h-2.5 overflow-hidden rounded-full bg-primy-100"><div className={`h-full rounded-full transition-[width] ${isPrimitiva ? 'bg-primy-600' : 'bg-eurodreams'}`} style={{ width: `${Math.max(4, Math.round(progress * 100))}%` }}/></div><div className="mt-4 flex justify-center gap-2" aria-hidden="true">{[1,2,3,4,5,6].map((n,index)=><span key={n} className={`number-ball primy-reveal flex h-9 w-9 items-center justify-center rounded-full border border-primy-200 font-display text-xs font-bold text-primy-800`} style={{animationDelay:`${index*70}ms`}}>?</span>)}</div></div>}
