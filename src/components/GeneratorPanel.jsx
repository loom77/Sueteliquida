  ...
  return (
    <section className="rounded-3xl border border-default bg-surface p-5 md:p-7">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-black text-indigo-700">Paso 2 de 2</p>
          <h2 className="mt-1 text-2xl font-black tracking-tight text-primary">Elige cuánto quieres jugar</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-secondary">Ajusta el presupuesto aumentando o reduciendo el número de columnas. Es el único control necesario.</p>
        </div>
      </div>
      ...
      {variantLabel && (
        <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-violet-200 bg-violet-50 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-black text-violet-950">Variante de una jugada existente</p>
            <p className="mt-1 text-sm leading-6 text-violet-800">Primy buscará columnas distintas de {variantLabel}.</p>
          </div>
          <button type="button" onClick={onClearVariant} className="min-h-11 rounded-xl px-4 text-sm font-black text-violet-800 hover:bg-violet-100">Quitar referencia</button>
        </div>
      )}

      <div className="mt-7">
        <GameSwitch active={activeGame} onChange={onGameChange} label="Juego elegido"/>
      </div>
      ...
      <div className="mt-6 rounded-3xl bg-muted p-5 sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-secondary">Presupuesto de la jugada</p>
        ...
        <div className="mt-6 flex items-center justify-between gap-4 rounded-2xl bg-surface p-3 sm:p-4">
          <button
            ...
            aria-label="Reducir el presupuesto en una columna"
          >
            −
          </button>
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm font-bold text-secondary">{columnCount === 1 ? 'columna' : 'columnas'} · {euro.format(game.price)} cada una</p>
          </div>
          ...
          <button
            ...
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-default text-2xl font-black text-primary hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Aumentar el presupuesto en una columna"
          >
            +
          </button>
        </div>
      ...
      >
        {busy ? `Preparando · ${Math.round(progress * 100)}%` : `Generar ${columnCount === 1 ? 'la jugada' : `${columnCount} columnas`} · ${euro.format(totalCost)}`}
      </button>
      <p className="mt-3 text-center text-xs leading-5 text-secondary">La generación no guarda ni compra nada.</p>

      {busy && (
        <div className="mt-4" role="progressbar" aria-label="Progreso de generación" aria-valuemin="0" aria-valuemax="100" aria-valuenow={Math.round(progress * 100)}>
      ...
      {exceedsLimit && (
        <div className="mt-4 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
          ...
          <p>Esta jugada superaría el límite mensual personal de <strong>{euro.format(monthlyLimit)}</strong>. Puedes reducir el presupuesto con el botón menos.</p>
        </div>
      )}
