  ...
  return (
    <section className="primy-panel primy-card-enter p-5 md:p-7" aria-busy={busy}>
      <div className="flex items-start gap-4">
        <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${theme.icon}`}><SparklesIcon width="22" height="22"/></span>
        <div><p className={`text-sm font-semibold ${theme.label}`}>Primy Core</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-primary">Prepara tu jugada</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-secondary">Elige el juego y cuántas columnas quieres incluir en este boleto.</p></div>
      </div>
        ...
        {compactLayout ? (
          <div className={`grid min-w-0 grid-cols-[104px_minmax(0,1fr)] items-center gap-4 rounded-3xl border p-3 sm:grid-cols-[120px_minmax(0,1fr)] ${theme.helper}`}>
            ...
            <div className="min-w-0 pr-1">
              <p className={`text-xs font-bold uppercase tracking-[0.16em] ${theme.label}`}>Primy te acompaña</p>
          ...
          <PrimyMascotGraphic className="w-full" variant={busy ? 'thinking' : 'helper'} size="dashboard" caption={busy ? 'Estoy coordinando tus columnas' : 'Elige tu presupuesto y yo preparo el resto'}/>
        )}
      </div>

      {busy && <div className="primy-core-progress mt-5"><div aria-live="polite"><p className="font-display text-lg font-semibold text-primary">{coreStage?.title}</p><p className="mt-1 text-sm leading-6 text-secondary">{coreStage?.detail}</p></div><div className="mt-4" role="progressbar" aria-label="Progreso de generación" aria-valuemin="0" aria-valuemax="100" aria-valuenow={Math.round(progress * 100)}><div className="h-2.5 overflow-hidden rounded-full bg-primy-100"><div className={`h-full rounded-full transition-[width] ${theme.progress}`} style={{ width: `${Math.max(4, Math.round(progress * 100))}%` }}/></div><div className="mt-4 flex flex-wrap justify-center gap-2" aria-hidden="true">{Array.from({ length: game.numbersToPick }, (_, index) => <span key={`main-${index}`} className="number-ball primy-reveal flex h-9 w-9 items-center justify-center rounded-full border border-primy-200 font-display text-xs font-bold text-primy-800" style={{animationDelay:`${index*70}ms`}}>?</span>)}{game.secondary && Array.from({ length: game.secondary.count }, (_, index) => <span key={`secondary-${index}`} className="primy-reveal flex h-8 w-8 items-center justify-center rounded-full border border-sky-300 bg-sky-50 text-xs font-bold text-sky-800" style={{animationDelay:`${(game.numbersToPick + index)*70}ms`}}>★</span>)}</div></div></div>}
