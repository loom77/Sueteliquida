...
test('home identifies the game and explains the preparation cutoff', () => {
  const home = read('../src/components/HomeExperience.jsx');
  const dashboard = read('../src/components/DashboardView.jsx');
  assert.match(home, /Próximo sorteo disponible/);
  assert.match(home, /nextDrawGame\.name/);
  assert.match(home, /GameIdentity gameId=\{nextDrawGame\.id\}/);
  assert.match(home, /Preparación disponible hasta las/);
  assert.match(dashboard, /getNextPlayableDrawInfo\('primitiva', now\)/);
});
