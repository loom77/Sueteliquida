...
test('todas las pantallas principales muestran la release 17.0.0', () => {
  assert.match(read('../src/utils/release.js'), /17\.0\.0/);
  assert.match(read('../src/components/AppShell.jsx'), /ReleaseStamp/);
  assert.match(read('../src/components/AuthScreen.jsx'), /ReleaseStamp/);
  assert.equal(JSON.parse(read('../package.json')).version, '17.0.0');
});
